...
Unfortunately, most metrics have been shown to be either unreliable or to require huge computational effort. Further, an evaluation of \gls{roar} in the like of \nameref{metrics:sanity-checks} has not been performed due to its computational complexity. This makes reliable comparison of methods almost impossible. For future research, evaluating \gls{roar} and finding new, more reliable and less computational complex metrics must be a priority. Proposing new methods is of no real worth, if we remain unable to evaluate against preexisting approaches.